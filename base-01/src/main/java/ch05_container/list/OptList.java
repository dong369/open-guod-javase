...
/**
 * 顺序性、唯一性、数据结构、算法、性能效率、多线程安全性、源码{@link ArrayList}、{@link Collections}
 ...
 * Vector：Object数组实现，但线程安全，效率略低和ArrayList实现方式相同, 但考虑了线程安全问题, 所以效率略低
 * LinkedList：双向循环链表实现，增删快，查找慢由于链表实现, 增加时只要让前一个元素记住自己就可以，删除时让前一个元素记住后一个元素，后一个元素记住前一个元素。
 * 这样的增删效率较高但查询时需要一个一个的遍历, 查询效率较低
 *
 * @author guod
 * @version 1.0
 * @since 1.8
